...
{
    if ([_scrollView viewWithTag:_currentPageIndex]) {
        return [_scrollView viewWithTag:_currentPageIndex];
    }
    return nil;
}
...
- (void)setPagesAtContentOffset:(CGPoint)offset{
    ...
    switch (orientation) {
        case PagedFlowViewOrientationHorizontal:{
            ...
            for (int i =(int) startIndex; i < [_cells count]; i++) {
                //如果都不超过则取最后一个
            ...
            for (int i =(int) endIndex + 1; i < [_cells count]; i ++) {
                [self removeCellAtIndex:i];
            }
            break;
        }
...
//                _scrollView.contentSize = CGSizeMake(8888888888,_pageSize.height);
...
- (void)scrollToPage:(NSUInteger)pageNumber animated:(BOOL) B {
    if (pageNumber < _pageCount) {
        switch (orientation) {
            case PagedFlowViewOrientationHorizontal:
                [_scrollView setContentOffset:CGPointMake(_pageSize.width * pageNumber, 0) animated:B];
                break;
            case PagedFlowViewOrientationVertical:
                [_scrollView setContentOffset:CGPointMake(0, _pageSize.height * pageNumber) animated:B];
                break;
        }
